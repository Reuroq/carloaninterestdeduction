"""Deploy carloaninterestdeduction.com — Render static site + Name.com domain.
Models the fleet pattern (_io_domain_setup.py). Secrets never printed.
Commands:
  recon          - check Render API/owner + Name.com availability & price
  create_service - create the Render static site from the GitHub repo
  register       - register the domain at Name.com
  dns            - add apex A + www CNAME records
  render_domain  - add custom domains to the service + trigger deploy
  status         - show custom-domain verification status
...
REPO   = "https://github.com/Reuroq/carloaninterestdeduction"
BRANCH = "main"
APEX_IP = "216.24.57.1"
SERVICE_ID = creds.get("CLI_SERVICE_ID", "")   # filled after create_service
...
def render_h(): return {"Authorization": f"Bearer {RENDER_KEY}", "Accept": "application/json"}
def nc_h():
...
def find_service():
    ...
    if st != 200: return None, (st, data)
    for item in data:
        svc = item.get("service", item)
        if "carloaninterest" in svc.get("name","").lower(): return svc, None
...
if __name__ == "__main__":
    ...
    if cmd == "recon":
        print("== RENDER owner/auth ==")
        st, data = req(f"https://api.render.com/v1/owners/{RENDER_OWNER}", headers=render_h()) if RENDER_OWNER else (0,{})
        st2, who = req("https://api.render.com/v1/services?limit=1", headers=render_h())
        print(f"  owner_id set={bool(RENDER_OWNER)} owner_http={st} services_http={st2}")
        svc, err = find_service()
        print(f"  existing carloan service: {svc.get('id') if svc else err}")
        print("== NAME.COM availability + price ==")
        st, data = req(f"{NC}/v4/domains:checkAvailability", method="POST", headers=nc_h(),
                       data={"domainNames": [DOMAIN]})
        if st == 200:
            for r in data.get("results", []):
                print(f"  {r.get('domainName')}: purchasable={r.get('purchasable')} "
                      f"price=${r.get('purchasePrice')} renew=${r.get('renewalPrice')} premium={r.get('premium')}")
        else:
            print(f"  check http={st} -> {json.dumps(data)[:300]}")

    elif cmd == "create_service":
        body = {
            "type": "static_site", "name": "carloaninterestdeduction",
            "ownerId": RENDER_OWNER, "repo": REPO, "branch": BRANCH, "autoDeploy": "yes",
        ...
        if st not in (200,201): print("  ->", json.dumps(data)[:500])
        ...
        www_target = sys.argv[2] if len(sys.argv) > 2 else "carloaninterestdeduction.onrender.com"
        ...
        for name in (DOMAIN, f"www.{DOMAIN}"):
            st, data = req(f"https://api.render.com/v1/services/{sid}/custom-domains",
                           method="POST", headers=render_h(), data={"name": name})
